...
interface SceneContextType {
  ...
  commentCount: number;
  likes: number;
  addComment: (message: string, isSystem?: boolean, emitToServer?: boolean) => void;
  triggerLike: () => void;
...
export function SceneProvider({ children }: { children: ReactNode }) {
  ...
  const { publicKey, userProfile } = useUser();
  ...
  useEffect(() => {
    ...
    socket.on(`${currentAgentId}_like_received`, () => {
      // console.log("like received for agent:", currentAgentId)
    ...
    socket.on(`${currentAgentId}_comment_received`, (data) => {
  ...
  const addComment = useCallback((
    ...
    const newComment = {
      ...
      message,
      avatar: isSystem ? randomAvatar : userProfile?.pfp || randomAvatar,
      handle: isSystem ? 'System' : userProfile?.handle || 'Anonymous',
      badges: randomBadges,
    ...
    if (scene) {
      ...
      const fetchGifts = async () => {
        try {
          ...
          const res = await axios.get(url);
          // console.log({ gifts: res.data.gifts })
        } catch (error) {
          console.error(`Failed to fetch gifts for ${currentAgentId}:`, error);
        }
      };
